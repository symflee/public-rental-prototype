...
`/admin/analytics`는 HTTP Basic 인증으로 보호됩니다. 이 화면은 선택한 날짜 범위의 실제 운영
조회와 동결된 8월 11~14일 재구성 조회를 하나의 서비스 지표로 통합합니다. 같은 날짜에 실시간
조회가 남아 있어도 동결된 재구성 실행을 기준값으로 사용해 중복 집계하지 않습니다. 8월 관측 기간은
`/admin/analytics?from=2026-08-11&to=2026-08-14`, 주택별 감사 내역은
`/admin/analytics/runs/2026-08-11-14`에서 확인합니다. 지도·주택 상세·공고 확인 행동 횟수와 현재
모집 중이 아닌 주택 상세 조회 비율을 함께 표시하며 북마크 관련 지표와 가설 판정은 표시하지 않습니다.
Neon
...
대상을 제약으로 삼아 결정적으로 재구성합니다. 따라서 DB와 주택별 감사 내역에는 `재구성 데이터`로
명시하며 실제 원시 로그라고 표현하지 않습니다. 적재 명령은 같은 데이터셋을 한 SQL 문에서 교체한
뒤 동결하므로 다시 실행해도 132건보다 늘어나지 않습니다.
