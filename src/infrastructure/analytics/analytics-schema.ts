export const ANALYTICS_SCHEMA_STATEMENTS = [
  `
    CREATE TABLE IF NOT EXISTS analytics_daily_counters (
      ...
      subject_kind text NOT NULL CHECK (subject_kind IN ('SITE', 'ANNOUNCEMENT', 'LOCATION')),
      subject_id text NOT NULL,
      total bigint NOT NULL DEFAULT 0 CHECK (total >= 0),
      PRIMARY KEY (metric_date, event_kind, subject_kind, subject_id),
      CONSTRAINT analytics_daily_counters_event_kind_check CHECK (event_kind IN (
        'PAGE_VIEW',
        'ANNOUNCEMENT_OPEN',
  ...
  `
    ALTER TABLE analytics_daily_counters
      ...
      ADD CONSTRAINT analytics_daily_counters_event_kind_check CHECK (event_kind IN (
        'PAGE_VIEW',
        'ANNOUNCEMENT_OPEN',
        'ANNOUNCEMENT_INTEREST',
        'OPEN_NOTICE_LOCATION_DETAIL_VIEW',
