export const LOCATION_DETAIL_VIEW_SCHEMA_STATEMENTS = [
  `
    CREATE TABLE IF NOT EXISTS analytics_runs (
      ...
      origin text NOT NULL CHECK (origin IN ('LIVE', 'RETROSPECTIVE_RECONSTRUCTION')),
  ...
  `
    CREATE TABLE IF NOT EXISTS analytics_location_detail_views (
      ...
      notice_state text NOT NULL CHECK (notice_state IN ('OPEN', 'NO_OPEN', 'UNKNOWN')),
      matched_notice_id text,
      status_source text NOT NULL CHECK (
        status_source IN ('AUTOMATED_IMPORT', 'MANUAL_REVIEW', 'SNAPSHOT_ABSENCE', 'UNKNOWN')
      ),
      origin text NOT NULL CHECK (origin IN ('LIVE', 'RETROSPECTIVE_RECONSTRUCTION')),
      created_at timestamptz NOT NULL DEFAULT now()
    )
